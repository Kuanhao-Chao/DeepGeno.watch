...
Task 4B: fix round 1/5 (`de1103aac1499ecca7b8ccc1e05fd4f30c92239b..34aaa2ffe3540002903696748fd732b355df9f7d`) enumerates and binds all relevant remote URLs with Bash 3.2, validates exact public metadata before mutation, performs a non-interactive empty-repository-safe HTTPS probe, resumes only exact untracked or exact staged nine-file seeds, and marks Preview URL Access pending until explicitly enabled and verified. RED was 4 failures/9 passes, followed by a separate non-interactive-probe RED of 1 failure/12 passes. GREEN is 39/39 focused and 185/185 full tests plus the complete local verification matrix; no remote or production mutation occurred. Scoped re-review passed with no Critical or Important findings.

Task 5: whole-branch review found three Important activation gaps: paid synthesis could be duplicated after an uncertain provider response, configured model access could not be probed without generation, and the approved 1/9/4 scan decisions were not reproducibly bound to the generated Gate 1 body. Standards review also found duplicated public schema validation and inconsistent Evidence Reference / Draft Summary / Published Summary terminology.

Task 5: implementation closes those findings with durable prepared/armed/dispatching/ambiguous/completed synthesis requests, a one-use execution token and stable provider idempotency key, a protected non-generative exact-model probe, a strict private controlled-scan compiler, one canonical public-paper schema, and Preview URL Access verification before Gate 2. The populated schema-v2 fixture build passed and the fixture was removed. Final whole-branch verification and the local implementation commit are complete; no remote or production mutation occurred.

Task 5: final semantic audit found one additional token-hygiene gap: an abnormal synthesis subprocess failure could repeat its raw one-use execution token in the rendered command or captured child diagnostics. A focused RED test reproduced the missing sanitizer; the fix now redacts both argv and captured output before emitting an error. Focused workflow verification is 28/28; the final full rerun passed 20 files / 205 tests plus typecheck, empty build, artifact/privacy checks, and Wrangler dry-run.
