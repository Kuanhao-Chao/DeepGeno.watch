...
abstract:
  Deep learning models exhibit empirical scaling laws whereby performance changes predictably with model size, dataset
...
hook:
  A systematic investigation of empirical scaling laws in single-cell transcriptomics reveals the conditions under which
...
coreProblem:
  statement:
    Unclear whether scaling laws established in NLP and vision translate reliably to noisy, high-sparsity single-cell
    RNA-seq [E0001].
  evidenceIds:
    - e1
novelty:
...
architecture:
  overview:
    "Scaling recipes for single-cell RNA sequencing foundation models: when do scaling laws hold? introduces Single-cell
...
takeaways:
  - statement:
      Scaling laws hold strongly for gene expression imputation and cell type annotation up to specific atlas complexity
      ceilings [E0001].
    evidenceIds:
      - e1
limitations:
