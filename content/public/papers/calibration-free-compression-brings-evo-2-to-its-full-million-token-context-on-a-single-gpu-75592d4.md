...
abstract: 'Evo 2 is the largest openly available genomic foundation model, but its forty billion parameter configuration cannot
...
hook: TurboQuant-Bio achieves near-lossless 4-bit compression of Evo 2 weights and attention cache, enabling 1M-token context
...
evidence:
  ...
  - id: e1
    ...
    locator:
      ...
      paragraph: 1
coreProblem:
  statement: Evo 2 40B cannot fit on a single 80 GB GPU and its chunked-prefill path silently produces uncorrelated likelihoods
    [E0001].
  evidenceIds:
  - e1
novelty:
...
- statement: Block-wise continuation algorithm repairing chunked-prefill to score 580 kb bacterial genomes in 22 min [E0001].
  ...
  - e1
architecture:
  overview: Calibration-free compression brings Evo 2 to its full million-token context on a single GPU introduces StripedHyena
...
- claim: Achieves state-of-the-art performance in dna-language-model [E0001].
  ...
  - e1
takeaways:
- statement: Near-lossless genomic perplexity, splice-site prediction, and variant effect prediction with 8x reachable context
    gain [E0001].
  evidenceIds:
  - e1
limitations:
